...
class makeModel_forDA(object):
    def __init__(self, arch=None, weights_file=None, model=None, device=-1):
        ...
        if model is not None:
            ...
            assert weights_file is None, 'Specify weight file'
            chainer.serializers.load_npz(weights_file, self.model)
        ...
        model = lock_layer(model)

    def lock_layer(self,model):
        return model
...
if __name__ == '__main__':
    ...
    #model = makeModel_forDA(arch=params['archs'][args.arch](), weights_file=args.initmodel, device=args.gpu)
